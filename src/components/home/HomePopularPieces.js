    ...
    <section className="bg-[#faf8f5] py-20 sm:py-28">
      <div className="mx-auto max-w-shop px-6 sm:px-10 lg:px-16">
        ...
        <div className="mt-12 grid grid-cols-1 gap-4 lg:grid-cols-12 lg:gap-5">
          <PieceTile product={hero} className="lg:col-span-7" large />
          <PopularSideColumn products={rest.slice(0, 2)} />
          {rest.length > 2 ? <PopularBottomRow products={rest.slice(2)} /> : null}
        </div>
      </div>
    </section>
  );
...
function PopularSideColumn({ products }) {
  return (
    <div className="grid grid-cols-2 gap-4 lg:col-span-5 lg:grid-cols-1 lg:gap-5">
      {products.map((p) => (
        <PieceTile key={p.slug} product={p} />
      ))}
    </div>
  );
}

function PopularBottomRow({ products }) {
  return (
...
function PieceTile({ product, className = "", large = false }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent px-4 pb-4 pt-16 sm:px-5 sm:pb-5">
